...
-- 1. Find all average of scores grouped by drivers.
SELECT driver_name, AVG(score) AS average_score
FROM responses
GROUP BY driver_name;

-- 2.Find all scores by department and driver combination
SELECT e.department, r.driver_name, AVG(r.score) as avg_score
FROM employees e
JOIN responses r ON e.id = r.employee_id
GROUP BY e.department, r.driver_name;


-- 3. Find all scores by location and driver combination
SELECT e.location, r.driver_name, AVG(r.score) as avg_score
FROM employees e
JOIN responses r ON e.id = r.employee_id
GROUP BY e.location, r.driver_name;

-- 4.Find all scores by gender and driver
SELECT e.gender, r.driver_name, AVG(r.score) as avg_score
FROM employees e
JOIN responses r ON e.id = r.employee_id
GROUP BY e.gender, r.driver_name;

-- 5.Find all scores by age group 20-30, 30-40,40-50
SELECT
    CASE
...
-- 6. Find all scores by managers (ensure manager name is there)
...
JOIN responses r ON e.id = r.employee_id
GROUP BY manager_name, r.driver_name
ORDER BY manager_name;



-- 7. Additionally, to ensure anonymity - if a group (eg: combination of department for a specific driver) has less than 3 responses - skip that group.
...
SELECT
  ...
  e.gender,
  CASE
    WHEN e.age >= 20 AND e.age < 30 THEN '20-30'
    WHEN e.age >= 30 AND e.age < 40 THEN '30-40'
    WHEN e.age >= 40 AND e.age < 50 THEN '40-50'
    ELSE 'Unknown'
  END AS age_group,
